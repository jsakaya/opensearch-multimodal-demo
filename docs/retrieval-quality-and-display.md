...
2. **Patch** records into retrievable evidence units: visual captions/assets,
   ...
   table cells.
3. **Index** pooled vectors, patch vectors, BM25 fields, facets, and table fields
   into OpenSearch.
...
vision/text inputs, but audio should not be represented as raw audio by default.
