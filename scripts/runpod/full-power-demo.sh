...
ssh -o StrictHostKeyChecking=accept-new -p "$PORT" root@"$IP" \
  "OPENLENS_TARGET_DOCS=${OPENLENS_TARGET_DOCS:-10000} OPENLENS_COLPALI_MAX_BATCH=${OPENLENS_COLPALI_MAX_BATCH:-16} OPENLENS_QWEN_MAX_BATCH=${OPENLENS_QWEN_MAX_BATCH:-64} bash -s" \
  < "$REMOTE_SCRIPT"
