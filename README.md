...
facets, modality-specific HNSW vectors, and per-document multi-vectors for
late-interaction reranking. Retrieval runs as keyword, vector, RRF hybrid, SQL,
or LIR patch rerank search. The code is designed so a tiny local corpus and a
million-document corpus use the same ingestion/index contract.
...
4. Store a pooled dense vector in `vector`, plus routed fields such as
   `qwen_vector`, `audio_vector`, `pdf_vector`, and `table_vector` for
   modality-specific HNSW k-NN.
5. Keep `patch_vectors` plus OpenSearch-docs-aligned `colbert_vectors` for
   late-interaction scoring.
6. Use `refresh_interval=1s` and `refresh=wait_for`/`refresh=true` on small upserts for near real-time retrieval.
7. Use streaming bulk indexing with deterministic IDs for scalable rebuilds and incremental replays.

The default `modality-router` embedder is deterministic and local so the 10k demo
can run anywhere while preserving the production vector dimensions. On GPU, set
`OPENLENS_USE_REAL_MODALITY_ENCODERS=1` to use Qwen3-VL for image/video, ColPali
for PDFs and visual patch reranking, and CLAP for audio-native retrieval. See
`docs/modality-specific-chunking.md` for the exact index contract.
...
For the old single-vector feature-hash baseline:

```bash
OPENLENS_EMBEDDING_BACKEND=feature-hash OPENLENS_VECTOR_DIM=128 uv run openlens-index
```
...
## Modality Router / RunPod Encoder

For the OpenSearch 3.6 modality router path on H100/H200:

```bash
uv sync --extra qwen --extra colpali --extra audio
...
3.6 service if `OPENSEARCH_URL` is not already reachable, autotunes ColPali and
Qwen batch sizes, builds the 10k NASA/space corpus, indexes 384-dimensional
common vectors plus modality-native Qwen/CLAP/ColPali fields into OpenSearch,
